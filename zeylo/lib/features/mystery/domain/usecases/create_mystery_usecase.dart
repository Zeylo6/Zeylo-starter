...
class CreateMysteryUseCase extends UseCase<MysteryEntity, CreateMysteryParams> {
  ...
  Future<Either<Failure, MysteryEntity>> call(
    ...
    final matchResult = await repository.matchMysteryExperience(newMystery);

    MysteryEntity mysteryToSave = newMystery;

    matchResult.fold(
      (failure) {
        // Ignore failure in matching and just save as pending
      },
      (matchedId) {
        if (matchedId != null) {
          mysteryToSave = newMystery.copyWith(
            status: MysteryStatus.matched,
            matchedExperienceId: matchedId,
          );
        }
      },
    );

    return repository.createMystery(mysteryToSave);
  }
}
