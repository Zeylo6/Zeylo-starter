...
class AdminReportsTab extends StatelessWidget {
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Pending Review', style: TextStyle(color: AppColors.error, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ],
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                         ...
                         Text('No pending reports to review.', style: AppTypography.bodyMedium.copyWith(color: AppColors.textSecondary)),
                      ],
                    ),
                  );
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final date = data['createdAt'] is Timestamp 
                      ? DateFormat.yMMMd().add_jm().format((data['createdAt'] as Timestamp).toDate()) 
                      : 'Unknown date';
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Container(
                                  ...
                                  child: Text(
                                    data['reporterRole']?.toString().toUpperCase() ?? 'UNKNOWN', 
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    data['reportedRole']?.toString().toUpperCase() ?? 'UNKNOWN', 
                                    style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600),
                                  ),
                                ),
                                const Spacer(),
                            ...
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('Reason: ', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.textSecondary)),
                                Expanded(child: Text(data['reason'] ?? 'Not specified', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 16))),
                              ],
                            ),
                            SizedBox(height: AppSpacing.sm),
                            if (data['details'] != null && data['details'].toString().isNotEmpty) ...[
                              Container(
                                ...
                                child: Text('"${data['details']}"', style: const TextStyle(fontStyle: FontStyle.italic)),
                              ),
                              SizedBox(height: AppSpacing.md),
                            ],
                            ...
                            Row(
                              ...
                              children: [
                                TextButton(
                                  onPressed: () => _dismissReport(context, doc.id),
                                  child: Text('Dismiss', style: TextStyle(color: AppColors.textSecondary)),
                                ),
                                SizedBox(width: AppSpacing.sm),
                                ElevatedButton.icon(
                                  onPressed: () => _takeActionDialog(context, doc.id, data),
  ...
  Future<void> _dismissReport(BuildContext context, String reportId) async {
    try {
      ...
      if(context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to dismiss: $e')));
      }
    }
  }

  void _takeActionDialog(BuildContext context, String reportId, Map<String, dynamic> reportData) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              await _executeAction(context, reportId, reportData, 'warning_email');
            },
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.error),
            child: const Text('Send Warning Email', style: TextStyle(color: Colors.white)),
          ),
        ]
      ),
    );
  ...
  static const String _backendUrl = 'http://10.0.2.2:3000'; // Android emulator -> localhost
  // static const String _backendUrl = 'http://localhost:3000'; // Web / iOS simulator

  Future<void> _executeAction(BuildContext context, String reportId, Map<String, dynamic> reportData, String actionType) async {
    try {
       // Field is stored as 'reportedUid' by report_sheet.dart
       final reportedUserId = reportData['reportedUid'] ?? reportData['reportedUserId'];
       ...
       await FirebaseFirestore.instance.collection('activities').add({
         ...
         'message': 'We have received a report regarding your recent conduct: "${reportData['reason']}". Please review our community guidelines. Further violations may result in suspension.',
         'type': 'admin_warning',
         'isRead': false,
         'createdAt': FieldValue.serverTimestamp(),
       });
       ...
       try {
         ...
         if (idToken != null) {
           ...
           if (response.statusCode == 200) {
             ...
             debugPrint('Email API responded with error: ${response.statusCode} - ${response.body}');
           }
         }
       } catch (emailError) {
