...
class ChainFormNotifier extends StateNotifier<ChainFormState> {
  ...
  Future<void> submitForm() async {
    ...
    // Extract the chain from the Right value
    final chain = result.getOrElse(() => throw Exception('unreachable'));
    ...
    try {
      ...
      for (final exp in state.experiences) {
        if (exp.hostId.isEmpty) {
          print('[Chain] Skipping experience "${exp.title}" — hostId is empty');
          continue;
        }
        ...
        bookingsCreated++;
        print('[Chain] Booking created for "${exp.title}" (host: ${exp.hostId})');
      }

      print('[Chain] Total bookings created: $bookingsCreated/${state.experiences.length}');
    } catch (e) {
      print('[Chain] Error generating chain bookings: $e');
      state = state.copyWith(
        isLoading: false,
        error: 'Chain created but failed to create bookings: $e',
      );
      return;
    }
