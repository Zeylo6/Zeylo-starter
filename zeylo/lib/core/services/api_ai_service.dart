...
class ApiAiService implements AIService {
  static const String baseUrl = 'http://10.0.2.2:3000/api/ai';
  ...
  Future<String> enhanceText(String prompt, String contextType) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('AI enhance failed: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
  ...
  Future<List<ChainExperience>> generateChainExperiences(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return chainArray.map((item) => ChainExperience(
          experienceId: item['experienceId']?.toString() ?? DateTime.now().millisecondsSinceEpoch.toString(),
          title: item['title']?.toString() ?? 'Generated Experience',
          startTime: item['startTime']?.toString() ?? '10:00',
          endTime: item['endTime']?.toString() ?? '12:00',
          duration: double.tryParse(item['duration']?.toString() ?? '2.0') ?? 2.0,
          price: double.tryParse(item['price']?.toString() ?? '0') ?? 0.0,
          isOvernight: item['isOvernight'] == true || item['isOvernight'] == 'true',
        )).toList();
      } else {
        throw Exception('AI Chain generation failed: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
   ...
   Future<Map<String, dynamic>> generateSurprise(Map<String, dynamic> preferences) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('AI Mystery generation failed: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
